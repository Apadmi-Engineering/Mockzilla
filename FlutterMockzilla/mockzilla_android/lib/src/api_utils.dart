...
extension BridgeHttpMethodBridge on BridgeHttpMethod {
  HttpMethod toDart() =>
      HttpMethod.values.firstWhere((element) => element.name == name);
}

@internal
extension HttpMethodBridge on HttpMethod {
  BridgeHttpMethod toBridge() =>
      BridgeHttpMethod.values.firstWhere((element) => element.name == name);
}

@internal
extension BridgeLogLevelBridge on BridgeLogLevel {
  LogLevel toDart() =>
      LogLevel.values.firstWhere((element) => element.name == name);
}

@internal
extension LogLevelBridge on LogLevel {
  BridgeLogLevel toBridge() =>
      BridgeLogLevel.values.firstWhere((element) => element.name == name);
}

@internal
extension BridgeDashboardOverridePresetTypeBridge
    on BridgeDashboardOverridePresetType {
  DashboardOverridePresetType toDart() => DashboardOverridePresetType.values
...
extension MockzillaConfigBridge on MockzillaConfig {
  BridgeMockzillaConfig toBridge() => BridgeMockzillaConfig(
    port: port,
    endpoints: endpoints.map((endpoint) => endpoint.toBridge()).toList(),
