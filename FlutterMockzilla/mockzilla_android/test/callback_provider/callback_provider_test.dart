...
void main() {
  group("Callback provider unit tests", () {
    ...
    test("endpointMatcher - with known, matching endpoint - returns true", () async {
      // Setup
      when(mockEndpoint.key).thenReturn("key");
      when(mockEndpoint.endpointMatcher).thenReturn((_) async => true);

      // Run test & verify
      expect(await sut.endpointMatcher(mockRequest.toBridge(), "key"), true);
    ...
    test("endpointMatcher - with known, non-matching endpoint - returns false",
      ...
      // Run test & verify
      expect(await sut.endpointMatcher(mockRequest.toBridge(), "key"), false);
      verify(mockEndpoint.endpointMatcher(mockRequest)).called(1);
    });
    ...
    test("endpointMatcher - with unknown endpoint - throws", () {
      ...
      // Run test & verify
      expect(
        () async => await sut.endpointMatcher(mockRequest.toBridge(), "unknown"),
    ...
    test("defaultHandler - with known endpoint - returns response", () async {
      // Setup
      when(mockEndpoint.key).thenReturn("key");
      when(mockEndpoint.defaultHandler).thenReturn((_) async => _responseFixture);
    ...
    test("errorHandler - with known endpoint - returns response", () async {
      // Setup
      when(mockEndpoint.key).thenReturn("key");
      when(mockEndpoint.errorHandler).thenReturn((_) async => _responseFixture);

      // Run test
      final result = await sut.errorHandler(_bridgeRequestFixture, "key");
    ...
    test("errorHandler - with unknown endpoint - throws", () async {
      // Setup
      when(mockEndpoint.key).thenReturn("key");
      when(mockEndpoint.errorHandler).thenReturn((_) async => _responseFixture);
      ...
      // Run test & verify
      expect(
        () async => await sut.errorHandler(_bridgeRequestFixture, "unknown"),
        throwsA(isA<EndpointNotFoundError>()),
      );
    });
    ...
    test("flutterDefaultHandler - with known endpoint - returns response", () async {
      // Setup
      when(mockEndpoint.key).thenReturn("key");
      when(mockEndpoint.defaultHandler).thenReturn((_) async => _responseFixture);
    ...
    test("flutterDefaultHandler - with unknown endpoint - throws", () async {
      ...
      // Run test & verify
      expect(
        () async => await sut.flutterDefaultHandler(_requestFixture, "unknown"),
        throwsA(isA<EndpointNotFoundError>()),
      );
    });

    test("flutterErrorHandler - with known endpoint - returns response", () async {
      // Setup
      when(mockEndpoint.key).thenReturn("key");
      when(mockEndpoint.errorHandler).thenReturn((_) async => _responseFixture);
